/*
 * Vehicles Server VehicleServer_spawn_PersistantVehiclesRandom
 *
 * Spawns Vehicles randomly, will check if a vehicle is already in that spot before spawning.
 *
 * Made by Andrew_S90
 */

private ["_main", "_type", "_dbID", "_currentVeh", "_targetVeh", "_center", "_classnames", "_damageMin", "_damageMax", "_blacklistPos", "_electro", "_classname", "_found", "_count", "_spawnPos"];
...
try 
{
	...
	{
		_electro = [10153.8,2456.5,0];
		_blacklistPos pushBack [_electro, 450];
	};
	...
	for "_i" from _currentVeh to (_targetVeh-1) do
	{
		...
		while {!_found || _count > 200} do
		{
			if(_type isEqualto "RandomBoats") then 
			{
				_spawnPos = [(_center select 0), 4000, (worldSize * 0.625), 10, 0, 0.25, 1, _blacklistPos,[0,0]] call BIS_fnc_findSafePos;
				if !((_spawnPos isEqualTo [0,0]) || (_spawnPos isEqualTo 0)) then
				{
					_spawnPos = [_spawnPos, 0, 150, 10, 2, 0.25, 0, _blacklistPos,[0,0]] call BIS_fnc_findSafePos;
				};
			}
			else
			{
				_spawnPos = [(_center select 0), 0, (_center select 1), 10, 0, 0.25, 0, _blacklistPos,[0,0]] call BIS_fnc_findSafePos;
			};
			if((_spawnPos isEqualTo [0,0]) || (_spawnPos isEqualTo 0)) then
			{
				_count = _count+1;
...
{
	format ["VehicleServer_spawn_PersistantVehiclesRandom: %1", _exception] call ExileServer_util_log;
};